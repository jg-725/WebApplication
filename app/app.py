...
def form_update_post(city_id):
    cursor = mysql.get_db().cursor()
    inputData = (request.form.get('fldName'), request.form.get('fldYear'), request.form.get('fldAge'),
                 request.form.get('fldMovie'), request.form.get('fldExtra'), city_id)
    sql_update_query = """UPDATE tblCitiesImport t SET t.fldName = %s, t.fldYear = %s, t.fldAge = %s, t.fldMovie = 
    %s, t.fldExtra = %s WHERE t.id = %s """
...
def form_insert_post():
    cursor = mysql.get_db().cursor()
    inputData = (request.form.get('fldName'), request.form.get('fldYear'), request.form.get('fldAge'),
                 request.form.get('fldMovie'), request.form.get('fldExtra'))
    sql_insert_query = """INSERT INTO tblCitiesImport (fldName,fldYear,fldAge,fldMovie,fldExtra) VALUES (%s, %s, %s, %s,%s) """
